...
const submit=()=>{
    ...
    axios.post('http://localhost:3040/user/adduser',details)
    .then((res)=>{
     console.log(res)
    })
    .catch((error)=>{
...
const search=(e)=>{
   ...
   const email= {
    ...
    axios.post('http://localhost:3040/user/search',email)
   .then((res)=>{
    seterror("")
     setuser(res.data)
   })
    ...
    <div className="modal-content">
        <div className='container'>
          ...
          <h5>Name: {user.firstName}{user.lastName}</h5>
           ...
           <button className='btn btn-warning' onClick={submit}>Add user</button>
        </div>:null}
