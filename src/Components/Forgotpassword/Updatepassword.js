...
function Updatepassword(props) {
    ...
    useEffect(() => {
        axios.post(`http://localhost:3040/user/passwordreset/${token}`)
            .then((res) => {
                if(!res){
                    navigate("/invalid")
                }
    ...
    const submit = (e) => {
        e.preventDefault()
        axios.post(`https://projectmangementtool-selvakumar-backend.onrender.com/user/resetpassword/${token}`, users)
            .then((res) => {
                seterror("")
                setsuccess(res.data)
                setload(true)
